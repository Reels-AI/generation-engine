...
class VideoSceneSplitter:
    ...
    def split_adaptive_video(self, video_path: str, output_path: Optional[str] = None, stats_file_name: Optional[str] = None, output_file_template: str = '$VIDEO_NAME-s-$SCENE_NUMBER.mp4') -> None:
        ...
        try:
            ...
            if len(scene_list) == 0:
                # No scenes detected, copy the original video to the output directory
                video_name = os.path.splitext(os.path.basename(video_path))[0]
                output_file_name = output_file_template.replace(
                    '$VIDEO_NAME', video_name).replace('$SCENE_NUMBER', '1')
                output_file_path = os.path.join(output_path, output_file_name)
                shutil.copy(video_path, output_file_path)
                logger.info(f"No scenes detected. Original video copied as {
                            output_file_name}")
            else:
                # Split video into scenes
                split_video_ffmpeg(video_path, scene_list,
                                   output_path, output_file_template)
                logger.info(
                    f"Video split into scenes and saved to {output_path}")
    ...
    def split_all_videos_in_directory(self, videos_dir: str, output_dir: Optional[str] = None, stats_file_name: Optional[str] = None, output_file_template: str = '$VIDEO_NAME-s-$SCENE_NUMBER.mp4') -> None:
        ...
        videos = [file for file in os.listdir(
            videos_dir) if file.lower().endswith(('.mp4', '.avi', '.mov', '.mkv'))]
        ...
        for video in videos:
            ...
            video_stats_file_name = stats_file_name if stats_file_name else f"{
                os.path.splitext(video)[0]}_stats.csv"

            self.split_adaptive_video(
                video_path, video_output_dir, video_stats_file_name, output_file_template)
    ...
    def clip_duration(self, videos_dir: str, metadata_file: str = 'metadata.json') -> None:
        ...
        clips = [file for file in os.listdir(videos_dir) if file.lower().endswith(
            ('.mp4', '.avi', '.mov', '.mkv'))]
        ...
        for clip in clips:
            ...
            try:
                with VideoFileClip(clip_path) as video_clip:
                    ...
                    self.metadata[clip] = {
                        "duration": duration, "sequence": sequence}
